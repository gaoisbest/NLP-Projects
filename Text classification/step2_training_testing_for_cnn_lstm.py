    ...
    for dirpath, dirnames, filenames in os.walk(os.path.join(data_dir_path, 'news_sohusite_unique_06_29')):
        ...
        for each_file in filenames:
            if each_file in considered_category.keys():
                ...
                curr_training_file = codecs.open(os.path.join(data_dir_path, 'news_sohusite_unique_for_cnn_06_29', 'training_' + considered_category.get(each_file) + '.cs'), mode='w', encoding='utf-8')
                curr_testing_file = codecs.open(os.path.join(data_dir_path, 'news_sohusite_unique_for_cnn_06_29', 'testing_' + considered_category.get(each_file) + '.cs'), mode='w', encoding='utf-8')

                for each_train in curr_file_articles[0:training_data_thres]:
                    curr_training_file.write(each_train)
                curr_training_file.close()

                for each_test in curr_file_articles[training_data_thres:(training_data_thres+testing_data_thres)]:
                    curr_testing_file.write(each_test)
                curr_testing_file.close()
