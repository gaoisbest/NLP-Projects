...
- for f in filter_sizes:
    ...
    - Max-pool `tf.nn.max_pool`
        ...
        - Output tensor: `(batch_size, 1, 1, num_filters)` `tf.reshape` to `(batch_size, num_filters)` 
- Concat output tensor for each filter_size to `(batch_size, len(filter_sizes)*num_filters)`
- FC1 with drop-out
