...
if __name__ == '__main__':
    ...
    MmCorpus.serialize(os.path.join(training_file_path, '01_16_corpus_12.mm'), corpus)
    #dictionary = Dictionary.load(os.path.join(training_file_path, '07_11_dictionary.dict'))
    corpus_tfidf_mm = MmCorpus(os.path.join(training_file_path, '01_16_corpus_12.mm'))
    ...
    # build the model
    lda = LdaModel(corpus=corpus_tfidf, id2word=dictionary, num_topics=50)
    index = MatrixSimilarity(lda[corpus_tfidf])
    ...
    final_results = codecs.open(os.path.join(training_file_path, 'lda_similarity.res'), mode='a', encoding='utf-8')
    ...
    for test_doc in codecs.open(os.path.join(training_file_path, 'sohu_text_similarity_testing.corpus'), mode='r', encoding='utf-8'):
        ...
        test_doc_vec_lsi = lda[test_doc_vec]
