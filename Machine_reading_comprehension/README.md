...
- Question & passage encoding
    - Word-level embedding
        - Pre-trained case-sensitive GloVe embeddings, [840B.300d](http://nlp.stanford.edu/data/glove.840B.300d.zip)
    - Character-level embedding (obtained by (BiDAF) or final hidden states of a bi-directional rnn applied to embeddings of characters in the token (R-Net and S-Net)), the character-level embeddings are generated by taking the ). Character-level embeddings is helpful to deal with OOV tokens.
